...
namespace Nezumikun {
  namespace WindowLed {
    ...
    EffectPerlinNoise::EffectPerlinNoise(Canvas & canvas, uint8_t framePerSecond, uint32_t timeInMilliseconds) : EffectWithTime (canvas, framePerSecond, timeInMilliseconds) {
      this->firstCall = true;
      this->x = random16();
      this->y = random16();
      this->z = random16();
    ...
    EffectPerlinNoise::~EffectPerlinNoise() {
      free(this->data);
    }
